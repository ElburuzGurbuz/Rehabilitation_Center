...
namespace CSE355BYS
{
    public partial class psychologist : System.Web.UI.Page
    {
        ...
        protected void add_psychologist(object sender, EventArgs e)
        {
            ...
            using (con)
            {
                using (SqlCommand cmd = new SqlCommand("sp_AddPsyhchologist", con))
                {
        ...
        protected void delete_psychologist(object sender, EventArgs e)
        {
            string connectionString = ConfigurationManager.ConnectionStrings["conStr"].ToString();
            ...
            using (con)
            {
                using (SqlCommand cmd = new SqlCommand("sp_DeletePsyhchologist", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    if (0 != cmd.ExecuteNonQuery())
                    {
                        Label1.Text = TextBox13.Text + " id'li psikolog veritabanından çıkarıldı!";
                    }
                    else
                    {
                        Label1.Text = "Veritabanına ekleme esnasında hata!";
